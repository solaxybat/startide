...
RE_AFTERNOON = re.compile(r"<afternoon>(.*?)</afternnon>", re.IGNORECASE)
RE_EVENING = re.compile(r"<evening>(.*?)</evening>", re.IGNORECASE)
RE_NIGHT = re.compile(r"<night>(.*?)</evening>", re.IGNORECASE)
# this map is just a faster way to select the right regexes (the first
# refex in each tuple will be prased, the following will always be weeded out)
...
class Room(DefaultRoom):
    ...
    Settings this room up using Extended Room with a custom
    description parser.
    """
    ...
    def at_object_creation(self):
        ...
        # this will be set dynamically. Can contain raw timeslot codes
        self.db.raw_desc = ""
    ...
    def return_appearance(self, looker):
        ...
        # As part of the Room Parent, format the output in a special way.
        if not looker:
            return
